...
import { SearchIcon, ShieldIcon, CopyIcon, ChevronDownIcon, ChevronUpIcon, FilterIcon, CheckBadgeIcon, LockIcon } from "lucide-react";
...
const RULES = rules;
const ALL_TAGS = Array.from(new Set(RULES.flatMap(r => r.tags))).sort();
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          <ShieldIcon className="w-6 h-6" aria-hidden />
        ...
        <div className="grid grid-cols-1 gap-4">
            ...
            return (
              <article key={rule.id} className="bg-white border border-neutral-200 rounded-2xl shadow-sm">
                <div className="p-4 flex items-start gap-3">
                  <div className="mt-1">
                    <LockIcon className="w-5 h-5" aria-hidden />
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {isOpen ? <ChevronUpIcon className="w-4 h-4" /> : <ChevronDownIcon className="w-4 h-4" />} Details
                    </button>
                  </div>
                </div>
                {isOpen && (
          ...
          {filtered.length === 0 && (
            <div className="text-center text-neutral-500 py-16">
              <CheckBadgeIcon className="w-8 h-8 mx-auto mb-2" />
